"""
Daily automation script — runs at 9am to:
  1. Fill in yesterday's game results and compute P&L
  2. Fetch today's schedule and odds
  3. Run the model on today's games
  4. Log all predictions (theoretical bets) to data/bets_log.csv
  5. Print a summary of recent performance + today's picks
...
_log_file = _LOG_DIR / f"daily_update_{date.today()}.log"
...
def score_and_log(today_games: pd.DataFrame, feat: pd.DataFrame,
    ...
    for _, row in today_merged.iterrows():
        ...
        if mkt is not None:
            ...
            # Apply quality gate filters (same as backtest)
